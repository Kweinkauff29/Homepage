(() => {
  ...
  function calendarEvents() { return state.filtered.length ? state.filtered : state.events; }
  ...
  function applyFilters(){const q=$('#eventSearch').value.trim().toLowerCase(),range=$('#dateRange')._flatpickr?.selectedDates||[],format=$('#formatFilter').value,ceOnly=$('#ceOnly').checked;state.filtered=state.events.filter(e=>{const hay=`${e.name} ${e.description} ${e.location}`.toLowerCase();if(q&&!hay.includes(q))return false;if(format!=='all'&&e.format!==format)return false;if(ceOnly&&!e.ce)return false;if(range[0]&&(!e.start||e.start<new Date(range[0].setHours(0,0,0,0))))return false;if(range[1]){const end=new Date(range[1]);end.setHours(23,59,59,999);if(!e.start||e.start>end)return false;}return true;});state.visible=12;renderUpcoming();renderCalendar();}
  function renderUpcoming(){const list=(state.filtered.length||$('#eventSearch').value||$('#formatFilter').value!=='all'||$('#ceOnly').checked||($('#dateRange').value||''))?state.filtered:state.events;$('#upcomingGrid').innerHTML=list.length?list.slice(0,state.visible).map(e=>card(e,false)).join(''):'<div class="ccor-empty">No matching upcoming items.</div>';$('#loadMore').hidden=list.length<=state.visible;$('#resultCount').textContent=`${list.length} upcoming ${PAGE_LABELS[PAGE].toLowerCase()} loaded`;}
  async function load(){
